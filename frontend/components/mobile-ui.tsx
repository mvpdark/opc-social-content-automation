...
export const MobilePanel = memo(function MobilePanel({ action, children, title }: { action?: ReactNode; children: ReactNode; title: string }) {
  return (
...
export const Metric = memo(function Metric({
  ...
  const toneClass = {
    ...
    coral: "bg-blush text-coral",
    green: "bg-sage text-moss"
  };
  return (
    <button
      className={[
...
export const TaskRow = memo(function TaskRow({
  ...
  return (
    <button
      ...
      type="button"
    >
      <div className="flex h-10 w-10 items-center justify-center rounded-[16px] bg-sage text-moss">
...
export const StepTile = memo(function StepTile({
  Icon,
  label,
  onClick,
  state,
...
export const ModeChip = memo(function ModeChip({
  active = false,
  label,
  onClick,
  testId
  ...
  return (
    <button
      ...
      className={[
        ...
        active
          ? "border-moss bg-moss text-white shadow-moss-sm"
...
export const SettingRow = memo(function SettingRow({
  ...
  return (
    <button
      className="flex min-h-[56px] w-full touch-manipulation items-center justify-between gap-3 rounded-full border border-sage bg-white px-4 py-3 text-left active:scale-[0.99] active:border-moss"
      data-testid={testId}
      onClick={onClick}
      type="button"
    >
      ...
      <span className="shrink-0 rounded-full bg-sage px-2.5 py-1 text-xs font-semibold text-muted">{state}</span>
    </button>
  );
});
