...
import { Loader2, RefreshCw, Save, ShieldCheck, Zap } from "lucide-react";

import { MemoizedTrendSourceCard, type MobileTrendContent } from "@/components/mobile-trend-source-review";

type BusyAction = "digest" | "job" | "link" | "search" | null;

interface CollectResultConfirmProps {
  pendingTrendItems: MobileTrendContent[];
  reviewedTrendIds: number[];
  trendListLoading: boolean;
  onRefreshTrendItems: () => void;
  trendListStatus: string;
  sourceReviewed: boolean;
  deletingTrendIdSet: Set<number>;
  onConfirmSingleTrendSource: (item: MobileTrendContent) => void;
  onDeleteTrendSource: (item: MobileTrendContent) => void;
  onOpenTrendItem: (item: MobileTrendContent) => void;
  onOpenTrendSourceUrl: (item: MobileTrendContent) => void;
  onToggleTrendSelection: (itemId: number) => void;
  reviewedTrendIdSet: Set<number>;
  selectedTrendIdSet: Set<number>;
  selectedTrendIds: number[];
  onConfirmSelectedTrendSources: () => void;
  busyAction: BusyAction;
  onSaveKnowledgeDigest: () => void;
  collectionBusy: boolean;
  activeCollectionJobId: number | null;
  onRunManualCollection: () => void;
  allPendingSelected: boolean;
  onClearTrendSelection: () => void;
  onSelectVisibleTrendItems: () => void;
  selectedPendingCount: number;
}

export const CollectResultConfirm = memo(function CollectResultConfirm({
  pendingTrendItems,
  reviewedTrendIds,
  trendListLoading,
  onRefreshTrendItems,
  ...
  sourceReviewed,
  deletingTrendIdSet,
  onConfirmSingleTrendSource,
  onDeleteTrendSource,
  onOpenTrendItem,
  onOpenTrendSourceUrl,
  onToggleTrendSelection,
  reviewedTrendIdSet,
  selectedTrendIdSet,
  selectedTrendIds,
  onConfirmSelectedTrendSources,
  busyAction,
  onSaveKnowledgeDigest,
  collectionBusy,
  activeCollectionJobId,
  onRunManualCollection,
  allPendingSelected,
  onClearTrendSelection,
  onSelectVisibleTrendItems,
  selectedPendingCount
}: CollectResultConfirmProps) {
  return (
    <section className="relative overflow-hidden rounded-[30px] border border-white/[0.92] bg-[rgba(255,253,247,0.88)] p-3 shadow-[0_18px_40px_rgba(31,58,49,0.09),inset_0_1px_0_rgba(255,255,255,0.96)] backdrop-blur-xl">
        ...
        {pendingTrendItems.length ? (
          pendingTrendItems.map((item) => (
            <MemoizedTrendSourceCard
              ...
              onConfirmSwipe={onConfirmSingleTrendSource}
              onDeleteSwipe={onDeleteTrendSource}
              onOpen={onOpenTrendItem}
              onOpenUrl={onOpenTrendSourceUrl}
              onToggle={onToggleTrendSelection}
              reviewed={reviewedTrendIdSet.has(item.id)}
              selected={selectedTrendIdSet.has(item.id)}
            />
          ))
        ) : (
          <div className="rounded-[24px] border border-dashed border-sage bg-white/[0.54] px-4 py-5 text-center text-xs font-semibold leading-5 text-muted">
            {trendListLoading
              ? "正在读取采集素材..."
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <button
          ...
          onClick={onConfirmSelectedTrendSources}
          type="button"
        >
