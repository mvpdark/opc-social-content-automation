  ...
  return text.split(/(\[[^\[\]]+\])/g).map((part, index) => {
    ...
    return (
      <span
        ...
        className="mx-0.5 inline-flex h-6 min-w-6 translate-y-[3px] items-center justify-center rounded-full border border-coral/30 bg-blush px-1.5 text-base leading-none shadow-sm"
        key={`${part}-${index}`}
