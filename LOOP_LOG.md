...
## Loop 88 - Review decision failure screenshot evidence

Date: 2026-06-16

### Observation

The mobile review decision failure E2E already proves failed human decisions keep the draft queued and avoid publishing-like calls. Its evidence is still text/assertion-only, so CI reviewers cannot quickly see that the user remains in a recoverable manual review detail state after approve/request-changes failures.

### Hypothesis

If the mobile review decision failure test attaches validated screenshots for both failed decision paths, then CI artifacts will make the manual-review safety boundary easier to audit without committing brittle pixel baselines.

### Patch

- Extended the mobile review decision failure E2E with `testInfo` screenshot attachments.
- Added visible status bounding-box checks for both failed approve and failed request-changes states.
- Attached validated `mobile-review-approve-failure.png` and `mobile-review-request-changes-failure.png` screenshots to the Playwright report.
- Added project verifier contracts for both decision-failure screenshot evidence points.
- Updated `PROJECT_MAP.md` to document that decision-failure screenshots are report artifacts and failed drafts remain queued.
...
cd frontend && npm run lint
python scripts\verify_project.py --keep-cache
cd frontend && npx playwright test tests/e2e/opc.smoke.spec.ts --grep "mobile review decision failure keeps draft queued without publishing" --project=chromium
cd frontend && npm run build
git diff --check
```
...
- `npx` is available at `11.12.1`.
- TypeScript check passed through `npm run lint`.
- Project verifier passed with `content_production_contract_checked=1334`.
- The focused Chromium E2E initially caught an over-tight status height threshold; after adjusting it to the actual rendered UI, the target test passed.
- Production build completed successfully for `/`, `/android`, and `/preview/[contentId]`.
- `git diff --check` passed.
...
Kept. CI can now attach visual evidence for mobile human-review decision failures while preserving the assertions that drafts stay queued and no publishing-like calls are made.
...
## Loop 91 - Mobile draft history recovery screenshot evidence

Date: 2026-06-16

### Observation

The mobile draft history read-error E2E already proves a history read failure can recover without source preview, content generation, image generation, or publishing-like calls. Its evidence is assertion-only, so CI reviewers cannot quickly inspect the visible mobile error and recovered draft card states.

### Hypothesis

If the mobile draft history recovery test attaches validated screenshots before and after retry, then CI artifacts will make the mobile history recovery path easier to audit without committing brittle image baselines.

### Patch

- Extended the mobile draft history read-error recovery E2E with `testInfo` screenshot attachments.
- Added visible bounding-box checks for the mobile history error card and the recovered draft history card.
- Attached validated `mobile-draft-history-error.png` and `mobile-draft-history-recovered.png` screenshots to the Playwright report.
- Added project verifier contracts for both mobile draft-history screenshot evidence points.
- Updated `PROJECT_MAP.md` to document that mobile draft-history retry screenshots are report artifacts and do not trigger generation or publishing calls.
...
- `npx` is available at `11.12.1`.
- TypeScript check passed through `npm run lint`.
- Project verifier passed with `content_production_contract_checked=1361`.
- The focused Chromium E2E initially caught an over-wide recovered-card threshold; after adjusting it to the actual mobile history card width, the target test passed.
- Production build completed successfully for `/`, `/android`, and `/preview/[contentId]`.
- `git diff --check` passed.
...
Kept. CI can now attach visual evidence for the mobile draft history read-error and recovery states while preserving assertions that no generation or publishing-like calls are made.
...
## Loop 97 - Public preview missing cover fallback coverage

Date: 2026-06-16

### Observation

Public preview now has invalid-link and valid-cover E2E coverage, but the product acceptance criteria also require cover preview failure/empty states to render safely. `PublicPreviewClient` already falls back to a text cover when `/image/list` fails or returns no usable image, but that branch is not protected by focused E2E coverage.

### Hypothesis

If a public-preview E2E mocks a successful draft response with a failed cover-list response, CI will catch regressions where a missing cover breaks the shared preview or hides the manual-review safety message.

### Patch

- Added a focused public-preview E2E for successful draft content with a failed cover lookup.
- Verified the text-cover fallback stays visible, the real cover image is absent, the title/body/tags and no-auto-publish message remain readable, and publishing-like endpoints are not called.
- Added project verifier contracts and updated `PROJECT_MAP.md` to document missing-cover public preview coverage.
...
python scripts\verify_project.py --keep-cache
cd frontend && npm run lint
cd frontend && npx playwright test tests/e2e/opc.smoke.spec.ts --grep "public preview keeps draft readable when cover lookup fails" --project=chromium
cd frontend && npm run build
git diff --check
git diff -- frontend\tsconfig.json
git status --short --ignored artifacts frontend\artifacts frontend\.next-build frontend\.next
...
- `npx` is available at `11.12.1`.
- Touched-file UTF-8 hygiene scan found no replacement characters or mojibake markers.
- Project verifier passed with `content_production_contract_checked=1425`.
- TypeScript check passed through `npm run lint`.
- Focused Chromium E2E passed for missing-cover public preview fallback.
- Production build completed successfully for `/`, `/android`, and `/preview/[contentId]`.
- `git diff --check` passed and `frontend/tsconfig.json` had no build-generated diff.
- Only ignored artifact/build directories are present under `artifacts/`, `frontend/.next-build/`, and `frontend/.next/`.
...
Kept. Public preview cover lookup failures now have CI-protected coverage that keeps draft text, tags, text-cover fallback, and the no-auto-publish safety message visible without invoking publishing-like endpoints.

### Remaining risk

- This loop covers cover-list failure after a successful content response; malformed image payloads and content backend failures can still use focused coverage.
- Screenshot/build artifacts remain ignored and are not committed.
...
## Loop 100 - Mobile exchange-rate custom fact topic alignment

Date: 2026-06-16

### Observation

Custom source-evidence topic checks cover market-data/pricing success paths and exchange-rate failure blocking, but the exchange-rate/currency-conversion custom topic does not yet have a successful mobile one-click generation path proving source evidence, generation payloads, preview, copy text, and manual review remain aligned.

### Hypothesis

If the mobile E2E runs an exchange-rate custom topic through source preview, one-click generation, draft preview, and copy assertions, CI will catch regressions where current exchange-rate topics bypass collected evidence or drift into generic planning content.

### Patch

- Added a mobile one-click E2E for the exchange-rate/currency-conversion custom fact topic.
- Verified source evidence is requested first, knowledge/web evidence lists contain the custom topic, generation payloads use the custom topic as `knowledge_query`, preview/copy output stays aligned, and publishing-like calls remain blocked.
- Added verifier contracts and updated `PROJECT_MAP.md` to document the exchange-rate custom topic smoke coverage.
...
python scripts\verify_project.py --keep-cache
cd frontend && npm run lint
cd frontend && npx playwright test tests/e2e/opc.smoke.spec.ts --grep "exchange-rate custom topic evidence aligned" --project=chromium
cd frontend && npm run build
git diff --check
git diff -- frontend\tsconfig.json
git status --short --ignored artifacts frontend\artifacts frontend\.next-build frontend\.next
...
- `npx` is available at `11.12.1`.
- Touched-file UTF-8 hygiene scan found no replacement characters or mojibake markers.
- Project verifier passed with `content_production_contract_checked=1456`.
- TypeScript check passed through `npm run lint`.
- Focused Chromium E2E passed for mobile exchange-rate custom topic evidence alignment.
- Production build completed successfully for `/`, `/android`, and `/preview/[contentId]`.
- `git diff --check` passed and `frontend/tsconfig.json` had no build-generated diff.
- Only ignored artifact/build directories are present under `artifacts/`, `frontend/.next-build/`, and `frontend/.next/`.
...
Kept. Mobile custom exchange-rate/currency-conversion topics now have CI coverage proving source evidence, generation requests, preview/copy output, tags, cover generation, and no-publishing guards stay aligned with the current-facts topic.

### Remaining risk

- This loop covers the mobile exchange-rate success path with mocked evidence and generation responses; PC exchange-rate success alignment can still be added.
- Live exchange-rate facts still depend on collected knowledge or the configured Tavily/web-search support path.
- Screenshot/build artifacts remain ignored and are not committed.
