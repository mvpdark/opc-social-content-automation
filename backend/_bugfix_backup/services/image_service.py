...
def list_images(db: Session, content_id: int | None, limit: int, user_id: int | None = None) -> list[GeneratedImage]:
    ...
    if user_id is not None:
        # Filter by content ownership via a subquery
        user_content_ids = select(Content.id).where(Content.user_id == user_id)
        statement = statement.where(GeneratedImage.content_id.in_(user_content_ids))
