...
def test_build_platform_search_target_encodes_keyword() -> None:
    ...
    assert target.requires_manual_login is True
    assert target.automation_mode == "operator_assisted_visible_browser"
...
def test_render_trend_knowledge_digest_includes_sources() -> None:
    payload = TrendKnowledgeDigestRequest(platform="douyin", keyword="博士申请", limit=5)
    item = TrendContent(
        id=7,
        platform="douyin",
        title="博士申请时间线",
